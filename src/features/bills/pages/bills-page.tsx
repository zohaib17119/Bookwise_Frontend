...
export function BillsPage() {
  ...
  const billsQuery = useBills(companyId, {
  ...
  const columns: DataTableColumn<Bill>[] = [
    ...
    { key: "status", header: "Status", render: (bill) => <DocumentStatusBadge status={bill.status} /> },
    {
      ...
      render: (bill) =>
        formatCurrency(bill.totals?.total ?? 0, bill.currencyCode ?? company?.currency ?? "USD"),
    },
    {
      key: "amountDue",
      header: "Amount Due",
      className: "text-right",
      render: (bill) =>
        formatCurrency(bill.amountDue ?? 0, bill.currencyCode ?? company?.currency ?? "USD"),
    },
  ];
  ...
  return (
    <EntityListPage
      ...
      toolbar={
        <FilterBar>
          <SearchInput onChange={setSearch} placeholder="Search bill number or reference" value={search} />
